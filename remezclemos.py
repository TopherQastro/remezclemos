...
def removeWordR(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine):  #  Remove the rightmost word from line
    ...
    if len(qLine[0]) > 0:
        ...
        qLineIndexList, proxDicIndexList = proxDataBuilder((runLine[0]+qLine[0], runLine[1]+qLine[1]), len(runLine[1]+qLine[1]))
...
def acceptWordR(empLine, superPopList, superBlackList, qLine, runLine, nextWord):  #  Add word to right side of line
    print('acceptWordR-in:', qLine, '|', nextWord)
    ...
    qLineIndexList, proxDicIndexList = proxDataBuilder((runLine[0]+qLine[0], runLine[1]+qLine[1]), len(runLine[1]+qLine[1]))
    #superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = superPopListMaker(empLine, superPopList, [], superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
    print('acceptWordR-out:', qLine, '|', nextWord, qLineIndexList, proxDicIndexList)
    return superBlackList, superPopList, qLineIndexList, proxDicIndexList, qLine
...
def superPopListMaker(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine): #  Creates a list-of-lists to pull from
    print(lineno(), 'sPLM init | len(superPopList)', len(superPopList))
    if len(superPopList) == (len(runLine[1]+qLine[1])+1):
    ...
    #qLineIndexList, proxDicIndexList = proxDataBuilder(testLine, len(testLine[1]))
...
##                    pWEmps = gF.empsLine(empLine, all, emps, doubles, quantumList)
##                    if (len(pWEmps) + len(pLEmps)) <= len(empLine):
##                        if (pLEmps+pWEmps) == empLine[:(len(pWEmps) + len(pLEmps))]:
            ...
            if len(keepList) == 0:
                ...
                if len(qLineIndexList) > 0:  #  Ensure that the line has something to check
                    if (len(qLineIndexList) <= proxMinDial):  #  This keeps the chain longer than a minimum length
                        pLEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = removeWordR(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
...
def testEmpLine(empLine, superBlackList, superPopList, qLineIndexList, proxDicIndexList, qLine, runLine, qWord):  #  A subfunction of metPopDigester, which tests words given to it
    ...
    if len(testEmps) <= len(empLine):  #  This is to screen against an error
        ...
        if testEmps == empLine[:len(testEmps)]:  #  Check if the word is valid
            ...
            hitSwitch = True
            superBlackList, superPopList, qLineIndexList, proxDicIndexList, qLine = acceptWordR(empLine, superPopList, superBlackList, qLine, runLine, qWord)
            print(lineno(), 'mPD acceptR', qLine, testEmps)
            return testEmps, superBlackList, qLineIndexList, proxDicIndexList, qLine, hitSwitch
    return testEmps, superBlackList, qLineIndexList, proxDicIndexList, qLine, hitSwitch


def metPopDigester(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine):  #  Digests words that fit a particular meter
    ...
    #qLineIndexList, proxDicIndexList = proxDataBuilder((runLine[0]+qLine[0], runLine[1]+qLine[1]), len(runLine[1]+qLine[1]))
    superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = superPopListMaker(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
    while len(superPopList) > 0:
        ...
        while len(superPopList[-1]) > 0:
            ...
            if (pWord in contractionList) and (pWord[:-2] != "'s") and (pWord[-1] != "'") and (pWord[:2] != ("o'" or "d'")):  #  This line will place contractions in a special list to be switched if nothing works
                ...
                testEmps, superBlackList, qLineIndexList, proxDicIndexList, qLine, hitSwitch = testEmpLine(empLine, superBlackList, superPopList, qLineIndexList, proxDicIndexList, qLine, runLine, qWord)
                if hitSwitch == True:
...
##                        print(lineno(), 'becomes:', synonym)
        ...
        if (len(qLine[1]) > 2):  #  We want qLine to have more than 2 words before trying punctuation because it sounds better, although it isn't necessary for function. Also, make sure to exhaust all other possibilities first
            ...
            if (punxCt == 0) and (len(punxList) > 0):
                ...
                superBlackList, superPopList, qLineIndexList, proxDicIndexList, qLine = acceptWordR(empLine, superPopList, superBlackList, qLine, runLine, ([pWord], [pWord]))
                print(lineno(), 'punxD acceptR', qLine)
                testEmps = gF.empsLine(empLine, qLine[0], emps, doubles, quantumList)                
                return testEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine
        if (len(qLineIndexList) > proxMinDial) and (len(runLine[1]+qLine[1]) > proxMinDial):
            print(lineno(), 'snip qLineIndex in:', qLineIndexList, proxDicIndexList, runLine[1], qLine[1])
            qLineIndexList = qLineIndexList[:-1]
            proxDicIndexList = proxDicIndexList[:-1]
            print(lineno(), 'snip qLineIndex out:', qLineIndexList, proxDicIndexList)
            superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = superPopListMaker(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
        elif len(qLine[1]) > 0 or len(runLine[1]) > 0: #and len(qLine[1]) > proxMinDial:  #  If we have enough words, then we can remove rightmost element and metadata, then try again
...
def firstWordSuperPopList(superBlackList):  #  Creates a superPopList that reloads the global firstWords list
    ...
    print(lineno(), 'len(superPopList):', len(superPopList), '| len(superPopList[0]):', len(superPopList[0]))
    return superPopList
...
def meterLiner(empLine, superBlackList, usedList, rhymeList, qLineIndexList, proxDicIndexList, qLine, qAnteLine):  #
    ...
    while pLEmps != empLine:  #  Keep going until the line is finished or returns blank answer
        ...
        if (len(runLine[1]) == 0) and (len(qLine[1]) == 0):  #  Check if we're starting with a completely empty line, load firstWords to superPopList if so
            ...
            pLEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = metPopDigester(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
            if len(superPopList[0]) == 0 and len(qLine[1]) == 0:
                print(lineno(), 'redButton == True')
                return superPopList, superBlackList, usedList, qLine, runLine, True #  redButton event
            ...
            if len(superPopList[0]) == 0 and len(qLine[1]) == 0:  #  Nothing seems to work
                ...
                return superPopList, superBlackList, usedList, qLine, runLine, True  #  redButton event, as nothing in the list worked
        ...
        if len(qLine[1]) > 0:  #  Make sure there's a line to analyze
            ...
            return [[]], superBlackList, usedList, qLine, runLine, True
        while len(pLEmps) > len(empLine):  #  If somehow the line went over the numbered lists
            (lineno(), 'meterLiner over emps')
            pLEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = removeWordR(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
            #superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = superPopListMaker(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
        if pLEmps == empLine:
            print(lineno(), 'pLEmps == empLine')
            print(lineno(), 'rhymeList:', rhymeList, 'superPopList:', len(superPopList), len(superPopList[-1]))
            if qLine[1][-1] in nonEnders:  #  Words that don't sound good as the last word of a line, such as conjunctions without something else to connect
                ...
                for all in allPunx:
                    if all in superPopList[-1]:  #  If puncuation fits, place one on the end of a line (will give the next line an easier start, too)
                        superBlackList, superPopList, qLineIndexList, proxDicIndexList, qLine = acceptWordR(empLine, superPopList, superBlackList, qLine, runLine, ([all], [all]))
                        break                        
...
def rhymeLiner(empLine, superBlackList, usedList, rhymeList, qLineIndexList, proxDicIndexList, qLine, qAnteLine):
...
##            expressList.append(all)
    ...
    for each in qAnteLine[1]:
        runLine[1].append(each)
    qLineIndexList, proxDicIndexList = proxDataBuilder((runLine[0]+qLine[0], runLine[1]+qLine[1]), len(runLine[1]+qLine[1]))
    superPopList, superBlackList, usedList, qLine, runLine, redButton = meterLiner(empLine, superBlackList, usedList, rhymeList, qLineIndexList, proxDicIndexList, qLine, runLine)  #  First, let it build a line, then if it doesn't happen to rhyme, send it back
    pLEmps = gF.empsLine(empLine, qLine[0], emps, doubles, quantumList)
    print(lineno(), 'redButton:', redButton)
    if redButton == True:
        return usedList, qLine, True
    if qLine[0][-1] in allPunx:
        pLEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = removeWordR(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)  #  Remove if end in puncuation
    if qLine[0][-1] not in rhymeList:
        pLEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = removeWordR(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)  #  Remove
        superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = superPopListMaker(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
    while qLine[0][-1] not in rhymeList or pLEmps != empLine:  #  Unless we find a rhyme to escape this loop, it'll subtract the word every time it gets to the beginning of the loop
        print(lineno(), 'rhymeLiner w/o rhyming line', qLine[0], pLEmps)
        #pLEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = removeWordR(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)  #  Remove the last
        #superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = superPopListMaker(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
        print(lineno(), 'rhymeList:', rhymeList, 'superPopList:', len(superPopList), superPopList[-1])
        while len(superPopList[-1]) > 0:  #  Let's see if there was a rhyme in our popList to add. If we don't return anything, it leaves this section like an moves on, like an implied "else"
            pWord = superPopList[-1].pop(random.choice(range(0, len(superPopList[-1]))))
            pLEmps = gF.empsLine(empLine, qLine[0]+[pWord], emps, doubles, quantumList)
            if (pWord in rhymeList) and (pWord not in nonEnders) and (pLEmps == empLine):  #  If there's a rhyme, then we can switch out the last word for that instead
        ...
        if (len(qLine[1]) > 0):  #  If it gets to this line, there was no rhyming matches
            print(lineno(), 'rhymeLiner out')
            if (len(qLineIndexList) > proxMinDial):
                qLineIndexList = qLineIndexList[:-1]
                proxDicIndexList = proxDicIndexList[:-1]
                print(lineno(), 'rhymeLiner proxData cut:', qLineIndexList, proxDicIndexList)
                superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = superPopListMaker(empLine, superPopList, rhymeList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)
            else:
                print(lineno(), 'rhymeLiner removeWord')
                pLEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = removeWordR(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)  #  Remove the last word
                superPopList, superBlackList, usedList, qLine, runLine, redButton = meterLiner(empLine, superBlackList, usedList, rhymeList, qLineIndexList, proxDicIndexList, qLine, runLine)  #  Here and below, meterLiner now has an expressList with the rhyming words, to increase their preference
                pLEmps = gF.empsLine(empLine, qLine[0]+[pWord], emps, doubles, quantumList)
        if redButton == True:
            return usedList, qLine, True            
##        else:
##            print(lineno(), 'rhymeLiner redButton')
##            return usedList, qLine,
        print(lineno(), 'rhymeLiner out', pLEmps)
        if pLEmps == empLine:
            if qLine[0][-1] in allPunx:
                pLEmps, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine = removeWordR(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine)  #  Remove if end in puncuation
                pLEmps = gF.empsLine(empLine, qLine[0], emps, doubles, quantumList)
...
##        return [], [], ([],[]), True
    ...
    if rhymeThisLine == True:
        ...
        if (len(rhymeList) > 0):  #  This dictates whether stanzaGovernor sent a rhyming line. An empty line indicates metered-only, or else it would've been a nonzero population
            usedList, qLine, redButton = rhymeLiner(empLine, superBlackList, usedList, rhymeList, qLineIndexList, proxDicIndexList, qLine, qAnteLine)
        else:
        ...
        superPopList, superBlackList, usedList, qLine, qAnteLine, redButton = meterLiner(empLine, superBlackList, usedList, rhymeList, qLineIndexList, proxDicIndexList, qLine, qAnteLine)
    else:
...
def main__init(defaultSwitch0, language0, accent0, textFile0, poemQuota0, stanzaQuota0, rhyMap0, empMap0, usedSwitch0, rhySwitch0, metSwitch0, thesSwitch0, proxMinDial0, proxMaxDial0, punxProxNum0):
    ...
    if defaultSwitch == True:  #  Preset values so you don't have to type everything every time you start the program
        ...
        thesSwitch = True
        rhyMap = 'aa'
        empMap = [[bool(0), bool(1), bool(0), bool(1), bool(0), bool(1), bool(0), bool(1), bool(0), bool(1)],
                  #[bool(0), bool(1), bool(0), bool(1), bool(0), bool(1), bool(0), bool(1), bool(0), bool(1)],
                  #[bool(0), bool(1), bool(0), bool(1), bool(0), bool(1), bool(0), bool(1), bool(0), bool(1)],
                  [bool(0), bool(1), bool(0), bool(1), bool(0), bool(1), bool(0), bool(1), bool(0), bool(1)]]
