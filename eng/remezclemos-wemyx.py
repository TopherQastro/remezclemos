...
def metPopDigester(empLine, superPopList, superBlackList, qLineIndexList, proxDicIndexList, qLine, runLine):  #  Digests words that fit a particular meter
    ...
    #$print(lineno(), "len(superPopList):", len(superPopList), "| len(superPopList[len(qLine[1])]):", len(superPopList[len(qLine[1])]), 'qLine:', qLine)
    while len(superPopList[len(qLine[1])]) > 0:
        #$print(lineno(), "len(superPopList[len(qLine[1])]):", len(superPopList[len(qLine[1])]))
