...
def dynaDataWriter(lang, dynaList, textFile, dynaType):
    dynaFile = gF.csv.writer(open(lang+'/data/textLibrary/textData/dynasaurus/'+textFile+'-'+dynaType+'.csv', 'w+'))
    dynaSaurus = {}
    for key, val in dynaList.items():
        svVal = str()
        for each in val:
            svVal = svVal+'^'
        dynaFile.writerow([key, svVal]) 
    #dynaFile.close()
    return dynaSaurus


def dynaDataOpener(lang, textFile, dynaType):
    dynaFile = gF.csv.reader(open(lang+'/data/textLibrary/textData/dynasaurus/'+textFile+'-'+dynaType+'.csv', 'r'))
    dynaSaurus = {}
    ...
    try:
        ...
        for all in gF.splitText:
            ...
            try:
                ...
                try:
                    syns = gF.nltk.wn.synsets(all)
                    for each in syns:
                        wordData = str(each).split("'")
                        synList = [str(lemma.name()) for lemma in gF.nltk.wn.synset(wordData[1]).lemmas()]
                        for syn in synList:
                            if (syn not in finalList) and (syn != all) and (len(syn) > 0) and (syn not in gF.allPunx):
